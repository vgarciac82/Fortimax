...
private static final Logger log = Logger.getLogger(ZipOutputStream.class);
	...
	/**
	 * Crea an nuevo ZIP output stream.
	 * @param out El output stream actual
	 * @param encoding El encoding a usar en el archivo y comentarios.
	 *
	 * <p>Para una lista de posibles valores ver <a
	...
	public void setComment(String comment) {
		//ejrv if (comment != null && comment.length() > 0xffff / 3 && getUTF8Length(comment) > 0xffff) {
		if (comment != null && comment.length() > 0xffff / 3 && getEncodedBytesLength(comment) > 0xffff) {
					...
					if ((e.flag & 8) == 0) {
						...
						if (e.size != def.getTotalIn()) {
							throw new ZipException(
								"invalid entry size (expected " + e.size + " but got " + def.getTotalIn() + " bytes)");
						}
	...
	private void writeLOC(ZipEntry e) throws IOException {
		...
		//ejrv byte[] nameBytes = getUTF8Bytes(e.name);
		byte[] nameBytes = getEncodedBytes(e.name);
	...
	private void writeCEN(ZipEntry e) throws IOException {
		...
		byte[] commentBytes;
		if (e.comment != null) {
			//ejrv commentBytes = getUTF8Bytes(e.comment);
			commentBytes = getEncodedBytes(e.comment);
	...
	private void writeEND(long off, long len) throws IOException {
		...
		if (comment != null) { // zip file comment
			//ejrv byte[] b = getUTF8Bytes(comment);
			byte[] b = getEncodedBytes(comment);
			writeShort(b.length);
	...
	*/
	...
	static int getEncodedBytesLength(String s) {
		int count = 0;
		try {
			count = s.getBytes(encoding).length;
		} catch (UnsupportedEncodingException ue) {	log.error(ue,ue);

		}
		return count;
	}

	private byte[] getEncodedBytes(String s) throws UnsupportedEncodingException {
