...
private static final Logger log = Logger.getLogger(LDAPManager.class);
	...
	private static Map<String, LDAPManager> instances = new HashMap<String, LDAPManager>();
	...
	protected LDAPManager(String hostname, int port, String username,
	...
	public void addUser(String username, String firstName, String lastName,
		...
		String cnValue = new StringBuffer(firstName).append(" ").append(
				lastName).toString();
	...
	public void deleteUser(String username) throws NamingException {
		...
		} catch (NameNotFoundException e) {	log.error(e,e);
	...
	public boolean isValidUser(String username, String password)
		...
		} catch (NamingException e) {	log.error(e,e);
	...
	public void assignUser(String username, String groupName)
		...
		} catch (AttributeInUseException e) {	log.error(e,e);
	...
	public void removeUser(String username, String groupName)
		...
		try {
			...
			context.modifyAttributes(getGroupDN(groupName), mods);
		} catch (NoSuchAttributeException e) {	log.error(e,e);
		...
		if (attributes != null) {
			...
			if (memberAtts != null) {
				for (NamingEnumeration<?> vals = memberAtts.getAll(); vals
						.hasMoreElements();) {
					if (username.equalsIgnoreCase(getUserUID((String) vals
							.nextElement()))) {
						return true;
					}
				}
	...
	public List<String> getGroups(String username) throws NamingException {
		...
		String filter = new StringBuffer().append("(&").append(
				...
				"(uniqueMember=").append(getUserDN(username)).append(")")
				.append(")").toString();
	...
	public boolean hasPermission(String groupName, String permissionName)
		...
		if (attributes != null) {
			...
			if (permAtts != null) {
				for (NamingEnumeration<?> vals = permAtts.getAll(); vals
						.hasMoreElements();) {
					if (permissionName
							.equalsIgnoreCase(getPermissionCN((String) vals
									.nextElement()))) {
						return true;
					}
				}
	...
	public List<String> getPermissions(String groupName) throws NamingException {
		...
		if (attributes != null) {
			...
			if (permAtts != null) {
				for (NamingEnumeration<?> vals = permAtts.getAll(); vals
						.hasMoreElements(); permissions
						.add(getPermissionCN((String) vals.nextElement())))
					;
			}
		}
	...
	public static DirContext getInitialContext(String hostname, int port,
			String username, String password) throws NamingException {
