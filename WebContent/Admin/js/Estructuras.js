Ext.onReady(function(){
	...
	var tituloColumna="Estructuras";
	var nombreIndex='Nombre';
	var definicionIndex='Definicion';
	var actionStore="getEstructuras";
	var tituloGrid="Estructuras";
	var tituloField="Estructuras";
	var tituloPanel="Estructuras";
	var tituloBtnCrear="Estructura";
	var ancho=500;
	var txtLabelFilas='Estructuras';
	var root='estructuras';
	var link_btncreate=basePath+'/Admin/jsp/EstructurasCrear.jsp?actualizar=false';
	var accionDescargar="getReport";
	/*
	 * Variables de Columnas
	 */
	var columnaActualizar='Actualizar';
	var linkActionColumnActualizar=basePath+"/Admin/jsp/EstructurasCrear.jsp?actualizar=true&nombre_estru=";
	var linkActionColumnActualizarMVC=basePath+"/Admin/jsp/EstructurasCrearMVC.jsp?actualizar=true&nombre_estru=";
	var columnaGavetas='Gavetas';
	var linkActionColumnGavetas=basePath+"/Admin/jsp/EstGavetas.jsp?nombre_estru=";
    ...
    fields: [
        ...
        {name: definicionIndex,  	type: 'string'}
    ]
	});
	...
	var labelFilas=new Ext.form.Label({
 		...
 		var btnDescargar = new Ext.ux.DownloadButton({
 		    ...
 		    select:'imx_catalogo_estructuras',
 		    nombreReporte:'Reporte Estructuras',
 		    columnaBuscar:'NOMBRE',
 		    Filtro:'txtFiltro',
		...
		columns:[
			  ...
			  { header: columnaActualizar, xtype: 'actioncolumn', width: 60, align  : 'center',items: [
			         ...
			         handler: function(grid, rowIndex, colIndex) {
			         	var definicion = storeEstructura.getAt(rowIndex).get(definicionIndex);
			         	if(definicion=='mvc')
			         		window.self.location.href = linkActionColumnActualizarMVC + storeEstructura.getAt(rowIndex).get(nombreIndex);
			         	else
			         		window.self.location.href = linkActionColumnActualizar + storeEstructura.getAt(rowIndex).get(nombreIndex);
			         }}	
			         ]
			   },
			   { header: columnaGavetas, xtype: 'actioncolumn', width: 60, align  : 'center',items: [
			   {icon   : '../imagenes/iconos/gaveta.png', cls:'actionColumns',tooltip: columnaGavetas,
			   getClass : function(){return 'actionColumns';},
			   handler: function(grid, rowIndex, colIndex) {
			   window.self.location.href = linkActionColumnGavetas + storeEstructura.getAt(rowIndex).get(nombreIndex);
			   }}	
			   ]
			   },
	...
	function Eliminar(Nombre){
		Ext.Msg.show({
		     title:'Eliminar',
		     msg: '&iquestDeseas eliminar la estructura: '+Nombre+' ?',
